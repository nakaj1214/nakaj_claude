...
## TDD の哲学と原則 (from test-driven-development)

### 核心原則

> テストが失敗するのを見ていなければ、それが正しいことをテストしているかわからない。

**失敗するテストなしに本番コードを書いてはならない。**

テストの前にコードを書いた場合は削除する。テストから新鮮に実装する。

### テストファーストの根拠

| よくある合理化 | 反論 |
|---|---|
| 「今回だけスキップしよう」 | それは合理化。Stop。 |
| 「時間がない」 | TDD は結果的に時間を節約する |
| 「プロトタイプだから」 | プロトタイプはユーザーに確認してから |
| 「参考のため残す」 | テスト前のコードは削除が鉄則 |

### 危険兆候

以下が見られたら TDD サイクルが壊れている:
- テストを後から書いている
- 実装詳細をテストしている（振る舞いをテストすべき）
- テストが遅い（1本 100ms 超）
- テストが順序に依存している
- モックが多すぎる（外部依存のみモックすべき）
