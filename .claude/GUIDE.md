...
├── SKILL.md          ← 概要のみ（30行以内）。ルーティング用
└── INSTRUCTIONS.md   ← 詳細手順。実行時にだけ読み込む
```
...
| スキル | トリガー | いつ使うか | 効果 |
|---|---|---|---|
| `brainstorming` | 「ブレスト」「要件整理」 | 要件が曖昧な時、設計の方向性を決めたい時 | ソクラテス式の問いかけで4フェーズの要件整理を実行 |
...
| `executing-plans` | 「計画実行」 | 書かれた計画を別セッションで実行する時 | レビューチェックポイント付きで実行 |
...
| `vba-excel` | 「Excel VBA」 | Excelワークシート操作・UserForm作成 | イベント処理・セキュリティパターン |
...
| `lint-on-save.py` | `.py` ファイルのEdit/Write後 | ruff format → ruff check --fix → ty check を自動実行。保存するだけでコード品質が保たれる |
...
| `edit-tracker.py` | Edit/Write後 | 編集ログを蓄積。learn-editsで分析に使う |
...
| やりたいこと | 使うもの |
|---|---|
| バグを調査したい | `/feature-dev` or `systematic-debugging` スキル |
| 新機能を計画したい | `/feature-dev` コマンド or `planner` エージェント |
| PRをレビューしたい | `/review-pr` コマンド |
| コミットを作りたい | `/commit` コマンド |
| 要件を整理したい | `brainstorming` スキル（「ブレスト」と言うだけ） |
| セッションを引き継ぎたい | `handover` スキル（または自動でPreCompactフックが生成） |
| Lintを自動化したい | `hooks/lint-on-save.py`（設定済み） |
| 外部API/DBと連携したい | `mcp/` テンプレートを参照してMCPサーバーを設定 |
| スキルを追加した後 | `python .claude/meta/generate-registry.py` |
| コンテキストが圧迫された | `/clear` でリセット（HANDOVERは自動生成される） |
| 複数エージェントを協調させたい | `/orchestrate` コマンド |
| フックを作りたい | `/hookify` コマンド |
| CLAUDE.mdを改善したい | `/revise-claude-md` コマンド |
