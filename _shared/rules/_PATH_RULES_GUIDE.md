...
`_shared/rules/` の内容を各プロジェクトにリンクできる:

```bash
# 共有ルールディレクトリをプロジェクトにリンク
ln -s /path/to/_shared/rules .claude/rules/shared

# 個別ファイルのリンク
ln -s /path/to/_shared/rules/security-rules.md .claude/rules/security.md
```
...
## このフォルダ（`_shared/rules/`）との使い分け

| | `_shared/rules/` | `.claude/rules/` |
|--|-----------------|-----------------|
| **役割** | テンプレート・参考 | プロジェクト実際のルール |
| **読み込み** | CLAUDE.md から `@` で参照 | Claude Code が自動検出 |
| **パス指定** | 不可 | `paths:` フロントマターで可 |
| **配布** | Git 経由で共有テンプレートとして | プロジェクトに直接コミット |
